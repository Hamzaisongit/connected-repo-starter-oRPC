...
// biome-ignore lint/correctness/noUnusedImports: used for debugging
...
export function CreateUserStackForm() {
	const renderCount = ++formRenderCount;
	
	const [success, setSuccess] = useState("");
	
	// biome-ignore lint/correctness/noUnusedVariables: used for debugging
	const prevRhfFormProviderRef = useRef<any>(null);
	...
	const prevFormMethodsRef = useRef(formMethods);
	
	// Track if formMethods object reference changed
	if (prevFormMethodsRef.current !== formMethods) {
		prevFormMethodsRef.current = formMethods;
	}
	
	// Track if RhfFormProvider component reference changed
	if (prevRhfFormProviderRef.current !== RhfFormProvider) {
		prevRhfFormProviderRef.current = RhfFormProvider;
	}
			...
			<Stack spacing={2}>
				...
				{/* Form Card */}
					...
					>
						...
						{/* Submit Button */}
						<RhfSubmitButton
							notSubmittingText="Confirm & Activate"
