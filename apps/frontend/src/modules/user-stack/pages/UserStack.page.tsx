...
import { UserStackCardView } from "@frontend/modules/user-stack/components/UserStackCardView";
import { UserStackEmptyState } from "@frontend/modules/user-stack/components/UserStackEmptyState";
import { UserStackTableView } from "@frontend/modules/user-stack/components/UserStackTableView";
...
export default function UserStackPage() {
	...
	const paginatedStacks = useMemo(() => {
		if (!userStacks) return [];
		const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
		const endIndex = startIndex + ITEMS_PER_PAGE;
		return userStacks.slice(startIndex, endIndex);
	}, [userStacks, currentPage]);

	const totalPages = useMemo(() => {
		if (!userStacks) return 0;
		return Math.ceil(userStacks.length / ITEMS_PER_PAGE);
	}, [userStacks]);
	...
	return (
		<Container maxWidth="xl" sx={{ py: { xs: 3, md: 5 } }}>
			...
			>
				<Box>
					...
					<Typography variant="body2" color="text.secondary" sx={{ fontSize: "0.875rem" }}>
						{userStacks.length} {userStacks.length === 1 ? "item" : "items"} in total
					</Typography>
				</Box>
			...
			<Box sx={{ mb: 4 }}>
				{viewMode === "card" ? (
					<UserStackCardView stacks={paginatedStacks} onStackClick={handleStackClick} />
				) : (
					<UserStackTableView stacks={paginatedStacks} onStackClick={handleStackClick} />
				)}
			</Box>
