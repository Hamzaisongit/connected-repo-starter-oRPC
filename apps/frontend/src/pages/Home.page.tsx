...
const HomePage = () => {
	...
	const { dailySchedule, dailyProgress, userStats, markAdherence } = useMockData();

	const handleCloseDialog = () => setSelectedSupplement(null);

	const handleTaken = () => {
		if (!selectedSupplement) return;
		markAdherence(selectedSupplement.id, selectedSupplement.scheduledTime, "Taken on-time");
		setSelectedSupplement(null);
	};

	const handleSkip = () => {
		if (!selectedSupplement) return;
		markAdherence(selectedSupplement.id, selectedSupplement.scheduledTime, "Skipped");
		setSelectedSupplement(null);
	};

	const handleSupplementClick = (supplementId: string, scheduledTime: number) => {
		const supplement = dailySchedule.find(
						...
						<Box sx={{
							...
							{[
								...
								<Box key={i} sx={{
									...
									flexDirection: 'column',
									alignItems: 'center',
									borderRight: i !== 1 ? '1px solid' : 'none',
									borderColor: 'divider'
								}}>
									{/* Colored Dot */}
