...
```
src/
├── modules/          # Features: auth, adherence-logs, streaks, logs, subscriptions, teams, users, webhook_calls
...
export class UserAdherenceLogsTable extends BaseTable {
...
export const userAdherenceLogCreateInputZod = userAdherenceLogMandatoryZod.extend(
...
import { userAdherenceLogCreateInputZod } from '@connected-repo/zod-schemas/user_adherence_log.zod';
import { db } from '../../db/db';

export const createAdherenceLog = rpcProtectedProcedure
  .input(userAdherenceLogCreateInputZod)
  .handler(async ({ input, context }) => {
    return db.userAdherenceLogs.create({
      ...input,
...
describe('Adherence Logs', () => {
  ...
  test('create', async () => {
    const log = await db.userAdherenceLogs.create({
      userId,
      supplementId,
...
export const createLog = rpcProtectedProcedure
  ...
  .handler(async ({ input, context }) => {
    return db.adherenceLogs.create({ ...input, userId: context.user.id });
  });
```
...
export const getUserLogs = rpcProtectedProcedure
  .handler(async ({ context }) => {
    return db.adherenceLogs.where({ userId: context.user.id });
  });
```
...
export const updateLog = rpcProtectedProcedure
  ...
  .handler(async ({ input, context }) => {
    const log = await db.adherenceLogs.findBy({ id: input.id });
    if (log.userId !== context.user.id) throw new Error('Unauthorized');
    return db.adherenceLogs.find(input.id).update(input);
  });
```
...
await db.$transaction(async (tx) => {
  await tx.adherenceLogs.create(logData);
